...
* More advanced options for the template. What would it take to allow the template writer to reuse one input multiple times? Could the input be given an identifier perhaps, then that identifier could be reused in curly braces instead of sqaure brackets?
* Can we get it working with multiple files at once? Hint: the file picker isn't currently configured to allow selecting multiple files. That might be something we can configure.